...
describe('classifyFailure', () => {
  it('detects rate limit from stderr', () =>
    expect(
      classifyFailure({ exitCode: 1, stderr: 'Error: 429 rate limit exceeded', timedOut: false }),
    ).toBe('rate_limit'));
  ...
  it('detects auth errors', () =>
    expect(classifyFailure({ exitCode: 1, stderr: '401 Unauthorized', timedOut: false })).toBe(
      'auth',
    ));
